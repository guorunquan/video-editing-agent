...
### Added
- 新增 `EditDraft` 草稿状态，保存方案片段、配乐、特效、预览、版本和确认状态。
- 方案卡支持选择本地配乐、音量与特效，并先生成保留源分辨率的视频预览。
- 未生成预览的草稿不能确认导出；预览与最终成片复用同一确定性 FFmpeg 渲染器。
- 本地音乐库支持 MP3/WAV/M4A/AAC/FLAC/OGG、循环、淡入淡出、原声混音和响度规范化。
- 新增轻量 BPM/节拍分析和 0.18 秒安全卡点。
- 新增淡入淡出、交叉渐变、慢放、快速放大、闪白和震动效果。
- gameplay 方案提示扩展为高燃卡点、操作还原和解说展示三个方向。

### Fixed
- “给整个原视频加高燃配乐/BGM/背景乐”现在会保留整段画面和原声、直接混入配乐并生成预览；不再被错误拆成先静音再无法加音乐。
- 过滤从 0 秒到结尾完整保留原片的无效“伪剪辑方案”；预览卡增加“不满意，返回原视频”；高燃混音提升 BGM 音量并适度压低游戏原声，使配乐清晰可听。
- 预览改为保留源视频分辨率和 CRF 20 画质，仅使用更快的编码预设加速生成，不再缩小到 960px。
- 方案默认根据风格和 BPM 自动选配乐；高燃方案内置一首程序生成的 150 BPM 免版权电子循环，无需联网也能混音、循环、淡入淡出和安全卡点。
- 补齐纯“方案一/二/三”和“给当前方案配上高燃音乐”的确定性对话入口，不再由模型误走旧确认流程或回答“没有配乐工具”。
- 修复视频分析失败、超时或文件处理失败时返回字符串，调用方却按二元结果解包而产生 `too many values to unpack` 的问题；现在所有分析路径都保持稳定返回结构并显示真实错误。
- 修复 v2.0 配乐与特效 JSON 示例在 Python f-string 中未转义花括号，导致构造分析提示词时触发 `Invalid format specifier` 的问题。
- 修复“采用/看看/预览方案 N”被旧剪辑工具截获、只出现确认卡但不生成视频的问题；现在这些表达都会创建 v2 草稿、实际渲染预览并把页面播放器自动切换到新视频，同时支持回复“确认”完成高清导出。
- 修复刷新页面或点“刷新预览”后播放器回退到原片，以及确认导出后仍停留在低清预览的问题。

### Deferred
- 逐帧击杀 OCR、自动目标跟踪、电影级光流、在线音乐抓取和完整多轨剪辑器延期到后续版本。
