...
面向：**下一个接手本仓库的 AI Agent 或人类开发者**（当前为 v1.8）。
...
截至当前，主线能力已完成 **v1.8 精确时间定位与媒体管理工作台**（以 `CHANGELOG.md` 的最新节为准）：
...
- 字幕修改使用 `edit_subtitles`，不要用 `add_text_overlay` 修改已有字幕；OpenCC 负责繁转简，分屏字幕按连续 SRT 片段整句匹配
...
v1.8 当前版本目标已完成。后续优先完善文本驱动剪辑、竖屏导出、移动水印跟踪。每次改动同步更新 CHANGELOG / USAGE / PROBLEMS。
```
